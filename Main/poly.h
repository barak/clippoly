...
#ifndef	POLY_H
#define	POLY_H	"$Header: /iue/cvsroot/vproject/wafer/mkwafer/clippoly/poly/poly.h,v 1.1.1.1 2000/09/12 09:34:32 binder Exp $"
...
// Deleted comments from Poly::Poly(const PolyPoint &) in-class defined
// constructor.
//
...
#endif
#ifndef SET_H
#include	"set.h"   // use local path not <xxx>. gsl 10/31/96
#endif
#ifndef POSADDER_H
#include	"posadder.h"   // use local path not <xxx>. gsl 10/31/96
#endif

int		intersect_right( const Edge &edge, const PolyPoint &point );

enum EdgeState { Unknown, None, Shared, Inside };
// enum LogicStates;
//class ostream; removed. gsl 5/1/97
class Vec;
...
public:
	...
	const PolyNode	*link() const
				{ return _link; }
	const PolyPoint	&point() const
				{ return p; }
	void setPoint(const PolyPoint& p_)
				{ p = p_; }
...
private:	
	...
	PolyNode( const PolyPoint &point, const Poly *parent, PolyNode *tail = 0)
	...
	PolyNode( const PolyNode *copy, const Poly *parent )
		...
		{ }
#endif
	PolyNode( const PolyPoint &point, const Poly *parent, EdgeState es )
...
private:
	...
	PolyNode		*list, *prev;
	Poly& operator=( Poly &copy );		// Don't use it!
...
//		{ }
	Poly( const PolyPoint &p, const Poly *parent, EdgeState es )
		: list( new PolyNode( p, parent, es ) ), prev(0)
		{ }
	Poly( const PolyPoint &p )
	...
	void	add( const PolyPoint &p, const Poly *parent, EdgeState es );
	void	add( const PolyPoint &p )
		{	add( p, 0, ::Unknown ); }	// 0 or this ???
			
	int	has_point( const PolyPoint &point ) const;	// point inside *this?
	...
	const PolyPoint		&firstpoint() const
					{ return list->point(); }
...
class PolyIter
{
	...
	PolyNode	*AppNext()
		{//PolyNode *dbg1 = poly.list,
			//*dbg2 = (app_next != 0) ? app_next : poly.list; //gsl 11/21/96
	...
	PolyNode	*add( const PolyPoint &point, int &new_point );
	PolyNode	*add( const PolyPoint &point )
...
public:
	...
	static int	add_point( PolyIter &a, PolyIter &b, const PolyPoint &p );
...
public:
	...
	const PolyPoint		&point() const
					{ return node()->point(); }
	const PolyPoint		&nextpoint() const
					{ return nextnode()->point(); }
	const PolyPoint		&linknextpoint() const
					{ return linkpoly().nextnode(link())->point(); }
	const PolyPoint		&linkprevpoint() const
...
public:
	...
	const PolyPoint		&point() const
					{ return node()->point(); }
	const PolyPoint		&prevpoint() const
