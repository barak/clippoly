#ifndef	PRIMITIVES_H
#define	PRIMITIVES_H	"$Header: /iue/cvsroot/vproject/wafer/mkwafer/clippoly/poly/primitvs.h,v 1.1.1.1 2000/09/12 09:34:32 binder Exp $"
...
//		{ hvec2_t res; v_fill2( x(), y(), 1.0, &res ); return res; }
		{ hvec2_t res; v_x(res) = x(); v_y(res) = y(); v_w(res) = 1;
			return res; }
	const PolyPoint &operator=( const hvec2_t &copy )
		{ _x = v_x(copy); _y = v_y( copy); return *this; }
	const PolyPoint &operator=( const PolyPoint &copy )
		{ _x = copy.x(); _y = copy.y(); return *this; }
};

inline PolyPoint	
operator+( const PolyPoint &p1, const PolyPoint &p2 )
{
	return PolyPoint( p1.x() + p2.x(), p1.y() + p2.y() );
}
...
inline void PolyPoint::setPt(PolyPoint p)
		{ _x = p.x(); _y = p.y(); } // added. gsl 2/14/97
			
inline double
len( const PolyPoint p )
{
	return sqrt( p.x() * p.x() + p.y() * p.y() );
}
 
inline PolyPoint	
operator/( const PolyPoint &p, double div )
{
	return PolyPoint( p.x() / div, p.y() / div );
}
...
operator==( const PolyPoint &p1, const PolyPoint &p2 )
{
    ...
    return ChkZero(len(p1 - p2)); // changed to a fuzzy Euclidean distance measure. gsl 11/21/96
}
...
double	angle( const PolyPoint &p1, const PolyPoint &p2, const PolyPoint &p3 );

inline double
atan( const PolyPoint &p )
{
...
point(const hvec3_t &p)
{
	...
	return PolyPoint( v_x(pn), v_y(pn) );
}
...
class PointList
{
	...
	PointList& operator=( PointList copy );
		
public:
	PointList( int nr = def_len );
	~PointList();
	
	void	add( const PolyPoint &add );
};
...
class Edge
{
	PolyPoint	point1, point2;
	// int		shared;
	
public:
	Edge( const PolyPoint &p1, const PolyPoint &p2 ) //, int share = -1 )
		: point1( p1 ), point2( p2 ) //, shared( share )
		{ }
	PolyPoint	middle() const
		{ return (point1 + point2) / 2.0; }
	// void	set_shared( int val )
	//	{ shared = val; }
	
	const PolyPoint	&p1() const
		{ return point1; } 
	const PolyPoint 	&p2() const
		{ return point2; }	
};
