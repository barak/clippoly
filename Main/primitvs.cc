static const char rcs_id[] = "$Header: /iue/cvsroot/vproject/wafer/mkwafer/clippoly/poly/primitvs.cc,v 1.1.1.1 2000/09/12 09:34:32 binder Exp $";
...
PointList::PointList( int nr )
	: len( nr ), cur( 0 ), points( new PolyPoint[ nr ] )
{
	assert( nr > 0 );
}
...
void
PointList::add( const PolyPoint &add )
{
	if (cur + 1 >= len)
...
double
angle(const PolyPoint &p1, const PolyPoint &p2, const PolyPoint &p3)
{
	PolyPoint	d1 = p1 - p2, d2 = p3 - p2;
	...
	assert( res >= 0 );        //removed. gsl 10/24/96
	assert( res <= M_PI * 2 ); //"
