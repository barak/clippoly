static const char rcs_id[] = "$Header: /iue/cvsroot/vproject/wafer/mkwafer/clippoly/poly/poly.cc,v 1.1.1.1 2000/09/12 09:34:32 binder Exp $";
...
#include	<iostream> //added. gsl 5/1/97
#include	"poly.h"
#include	"posadder.h"
...
void
Poly::add( const PolyPoint &p, const Poly *parent, EdgeState edgestate )
{
	if (!prev)
		make_prev();
...
Poly::orientation() const
{
	...
	double	tot_angle = 0;
	double	area = 0;
	//PolyPoint	first;             removed. gsl 1/29/97
	//int	first_flag = 1;    "
...
errors than original routine. Created method setPt in PolyPoint class. gsl 2/14/97
*/
...
Poly::orientation() const
{
	...
	PolyPoint q(0,0), // central vertex
        p1(0,0),
        p2(0,0);
...
Poly::revert()
{
  ...
  PolyNode	*next = NULL; /* Added initialization to NULL. Otherwise compiler complains that next is
				 ...
				 conceivably crash the applicaiton. gsl 10/11/96 */
...
int
intersect_right( const Edge &edge, const PolyPoint &point )
{
...
int	
Poly::has_point( const PolyPoint &point ) const
{
...
PolyNode *	
PolyIter::add( const PolyPoint &point, int &new_point )
{
  PolyIter	chk(poly);
  ...
  while(chk())
	...
	assert( d <= len( cur->p - AppNext()->p ) ); //removed. gsl 10/24/96
...
int	
PolyIter::add_point( PolyIter &a, PolyIter &b, const PolyPoint &p )
{
  int	res = 0;
...
Poly::intersect_table( int hor, Vec &intersection_table, double h )
{
	...
	PolyPoint	left( hor ? bbox.xmin()-1 : 0, hor ? 0 : bbox.ymin()-1 ), 
