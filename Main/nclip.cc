static const char rcs_id[] = "$Header: /iue/cvsroot/vproject/wafer/mkwafer/clippoly/poly/nclip.cc,v 1.1.1.1 2000/09/12 09:34:32 binder Exp $";
...
#include	<iostream.h>   // added. gsl 5/1/97
...
#ifdef DEBUG1
#include	<iostream.h>
#include	"polyio.h"     // use local path not <xxx>. gsl 10/31/96
#endif
#ifdef DEBUG_NCLIP
#include        "polyio.h"
#include        <iostream.h>
...
int		
  intersect( const Edge &a, const Edge &b, PolyPoint &p1, PolyPoint &p2 )
{
  hvec2_t	s1, s2;
  ...
  while( a_iter() )
    {
      ...
      while( b_iter() )
	{
	  PolyPoint	intersect1, intersect2;
...
void
  label_shared2( PolyIter &pi, const PolyPoint &p1, const PolyPoint &p2, const PolyNode &pn )
{
  assert(p1 == pn.point());
  
  PolyPoint	prev = pn.prevnode().point(),
  		next = pn.nextnode().point();
  
  // The *dir are direction vectors!
  PolyPoint		cdir = p2 - p1, pdir = p1 - prev, ndir = next - p1;

  int	p_c = (pdir.x() * cdir.y() - pdir.y() * cdir.x()) >= 0, //removed. gsl 10/24/96
  	n_c = (ndir.x() * cdir.y() - ndir.y() * cdir.x()) >= 0, //"
  	p_n = (pdir.x() * ndir.y() - pdir.y() * ndir.x()) >= 0; //"
  //int	p_c = (pdir.x() * cdir.y() - pdir.y() * cdir.x()) > 0 || ChkZero(pdir.x() * cdir.y() - pdir.y() * cdir.x()), //added. gsl 10/24/96
  //	    n_c = (ndir.x() * cdir.y() - ndir.y() * cdir.x()) > 0 || ChkZero(ndir.x() * cdir.y() - ndir.y() * cdir.x()), // "
  //	    p_n = (pdir.x() * ndir.y() - pdir.y() * ndir.x()) > 0 || ChkZero(pdir.x() * ndir.y() - pdir.y() * ndir.x()); // "
...
void
make_poly( const PolyPoint &point, 
...
int 
make_poly( const PolyPoint &start_point, const PolyPoint &point, 
	DirPolyIter &follow, NodePEdgeList &done, Poly * new_poly )
{
	...
	//if (follow.point() == start_point) removed to fix zt bug; self-intersecting output poly
	//	return 1;                        gsl 1/23/97
			
	double	cur_angle = angle( point, follow.point(), follow.nextpoint() );

	//assert(cur_angle != 0); removed. gsl 10/24/96
	assert(! ChkZero(cur_angle)); //added. gsl 10/24/96
	...
	if (follow.link())
	{
		...
		PolyPoint	d1 = follow.linknextpoint() - follow.point(),
		...
		/*assert((cur_angle != other_angle) ||
		       (inp != 0) || removed. gsl 10/24/96*/
		assert(! ChkZero(cur_angle - other_angle) ||
		       (inp != 0) || //added. gsl 10/24/96, changed 11/20/96
		       (follow.linknextpoint() == follow.nextpoint()));
		/*assert((other_angle != 0) ||
		       (inp2 != 0) || removed. gsl 10/24/96*/
		assert(! ChkZero(other_angle) ||
		       (inp2 !=0) || //added. gsl 10/24/96, changed 11/20/96
	...
	if (dir != NONE)
	{
		...
		if (follow.point() == start_point) //added if's to fix zt bug; self-intersecting output poly gsl 1/23/97
...
poly_min_poly( const Poly &a, const Poly &b, PolyPList &a_min_b )
{
	...
	PolyPoint			top(b.firstpoint()), bottom(b.firstpoint());
...
void	
add_until( Poly *polyp, const PolyPoint &point, DirPolyIter &dpi )
{
	do
	{
		polyp->add(dpi.point());
		...
		PolyPList &a_min_b, PolyPList &b_min_a, PolyPList &a_and_b )
{
	...
	if (nr_intersect == 0)
	{
		// Two case are possible: a and b are disjoint, 
