static char rcs_id[] = "$Header: /usr/local/src/lib/math/graphmat/RCS/graphmat.c,v 1.8 1993/01/28 15:25:47 klamer Exp $";
/*
  $Log: graphmat.c,v $
...
/*} error routine removed. not used in MatLab MEX function. gsl 10/30/96 */

int 
#ifdef __cplusplus
gsl_error(char *errStr)
#else
gsl_error(errStr)
char *errStr;
#endif
{
#ifdef mac_h /* added for IBM PC compatibility. gsl 6/6/97 */
    mexPrintf(errStr);
#endif
    assert(1==0);
    return 1; /* added to satisfy compiler. gsl 10/30/96 */
}
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for m_cpy2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for m_unity2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_cpy2.");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_fill2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_unity2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_zero2().");
...
#endif
{
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for m_cpy3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for m_unity3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_cpy3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_fill3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_unity3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_zero3().");
...
#endif
{
   ...
   while(m_elem(*m_input, nextelement, rownr) == 0.0) {
     /*if(++nextelement == 3)
        gm_error(MATSING, "m_inv2()"); removed. gsl 10/30/96 */
     assert(++nextelement != 3); /* added. gsl 10/30/96 */
   }
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for m_inv2().");
...
#endif
{
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for m_tra2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for mm_add2().");
...
#endif
{
   ...
   { 
     ...
     {
        m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for mm_mul2().");
        which_matrix = 0;
     };
   ...
   switch(which_matrix)
   {
	...
	/*error("This should not happen! %s %d\n", __FILE__, __LINE__ ); removed. gsl 10/16/96 */
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for mm_sub2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for mtmm_mul2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for sm_mul2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for vvt_mul2().");
...
#endif
{
   ...
   {
     v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for mv_mul2().");
     inplace = 0;
   };
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for sv_mul2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_homo2().");

   v_x(*v_result) = gm_DIV(v_x(*vector), v_w(*vector), "Division by zero in v_homo2().");
...
#endif
{
   ...
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for v_norm2().");

   v_x(*v_result) = gm_DIV(v_x(*vector), length, "Division by zero in v_norm2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for vv_add2().");
...
#endif
{
   v_result = gm_ALLOC(hvec2_t, v_result, "Cannot allocate memory for vv_sub2().");
...
#endif
{
   ...
   while(m_elem(*m_input, nextelement, rownr) == 0.0) {
     /*if(++nextelement == 4)
        gm_error(MATSING, "m_inv3()"); removed. gsl 10/30/96 */
     assert(++nextelement != 4); /* added. gsl 10/30/96 */
   }
...
#endif
{
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for m_tra3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mm_add3().");
...
#endif
{
   ...
   { 
     ...
     {
        m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mm_mul3().");
        which_matrix = 0;
     };
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mm_sub3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mtmm_mul3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for sm_mul3().");
...
#endif
{
   ...
   {
     v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for mv_mul3().");
     inplace = 0;
   };
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for sv_mul3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_homo3().");

   v_x(*v_result) = gm_DIV(v_x(*vector), v_w(*vector), "Division by zero in v_homo3().");
   v_y(*v_result) = v_y(*vector) / v_w(*vector);
...
#endif
{
   ...
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for v_norm3().");
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for vv_add3().");
...
#endif
{
   ...
   if(v_result == vectorA)
   {
     ...
     {
        v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for vv_cross3().");
        which_vec = 0;
     };
...
#endif
{
   v_result = gm_ALLOC(hvec3_t, v_result, "Cannot allocate memory for vv_sub3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for vvt_mul3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for miraxis2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for mirorig().");
...
#endif
{
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for rot2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for scaorig2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat2_t, m_result, "Cannot allocate memory for transl2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for miraxis3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mirorig3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for mirplane3().");
   for(i=0; i<4; i++)
     for(j=0; j<4; j++)
        m_elem(*m_result, i, j) = (i != j) ? 0.0 : 1.0;
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for prjorthaxis().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for rot3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for scaorig2().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for scaplane3().");
   for(i=0; i<4; i++)
     for(j=0; j<4; j++)
        m_elem(*m_result, i, j) = (i != j) ? 0.0 : 1.0;
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for scaxis3().");
...
#endif
{
   ...
   m_result = gm_ALLOC(hmat3_t, m_result, "Cannot allocate memory for transl2().");
