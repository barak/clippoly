static const char rcs_id[] = "$Header: /home/mi7/klamer/src/c++/spie/RCS/graphadd.cc,v 1.1 1994/01/04 12:55:37 klamer Exp klamer $";
...
// split C and C++ parts.
//
// Uncommented code in v_inters2 that I should
// not have removed related to l_p and l_q
// variables. gsl 1/20/97
//
// 				     GRAPHADD.CC
//
//
...
//#include	<assert.h> // removed. added assert define in precompiled header. gsl 10/29/96
  ...
  operator==( const hvec2_t &h1, const hvec2_t &h2 )
{
  ...
  //return ChkZero(v_x(h1) - v_x(h2)) && ChkZero(v_y(h1) - v_y(h2)); //added. gsl 10/24/96. removed 11/22/96
  return ChkZero(m_len(h1 - h2)); // changed to a fuzzy Euclidean distance measure. gsl 11/22/96
...
{
  //return (v_x(h1) != v_x(h2)) || (v_y(h1) != v_y(h2)); removed. gsl 10/24/96
  //return ! ChkZero(v_x(h1) - v_x(h2)) || ! ChkZero(v_y(h1) - v_y(h2)); //added. gsl 10/24/96, removed. gsl 11/22/96
  return ! ChkZero(m_len(h1 - h2)); // changed to a fuzzy Euclidean distance measure. gsl 11/22/96
			 ...
			 const hvec2_t & q1, const hvec2_t &q2, hvec2_t &ret )
{
  ...
  while ((m != s1) && (m != s2))
  {
    ...
    //assert(len(m+q1 - q2) <= len(q2 - q1)); "
    assert( ! (len(m) > len(q2 - q1)));         //added. gsl 10/23/96, changed 11/22/96
    assert( ! (len(m+q1 - q2) > len(q2 - q1))); // "
...
*******************************************************************/
{
	...
	assert(p1 != p2); // Degenerate segment. Both vertices are the same. gsl 11/22/96
	assert(q1 != q2); // "
